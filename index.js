...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'inventory') {
    ...
    if (!items.length) return interaction.reply("No registered assets.");
  ...
  if (interaction.commandName === 'remove-item') {
    ...
    inventories[target.id] = inventories[target.id].filter(i => i !== itemName);
  ...
  if (interaction.commandName === 'buy') {
    ...
    if (location === 'medpoint') {
      ...
      if (accounts[userId] < price) return interaction.reply("Insufficient credits.");
...
client.login(token);  new SlashCommandBuilder()
    .setName('grant-item')
    .setDescription('Register an asset to a user')
    .addUserOption(option => option.setName('user').setDescription('User').setRequired(true))
    .addStringOption(option => option.setName('item').setDescription('Item name').setRequired(true)),
